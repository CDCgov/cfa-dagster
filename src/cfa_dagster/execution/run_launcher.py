...
class DynamicRunLauncher(RunLauncher, ConfigurableClass):
    ...
    # NOTE: This only resolves launcher config and not executor config. The run launcher cannot reliably resolve executor config because of the following:
    # 1. The config schema generated by Definitions(executor=dynamic_executor) isn't exposed via remote repo
    # 2. The RunConfig isn't resolved with the schema defaults provided in #1 until the executor is created
    def _get_launcher_config(
        ...
        if context and not launcher:
            ...
            if metadata_config:
                ...
                log.debug(f"After metadata config: launcher={launcher}")
        ...
        if not launcher:
            ...
            if legacy_tags.get("class"):
                ...
                log.debug(f"After legacy tags: launcher={launcher}")
        ...
        if context and not launcher:
            ...
            if legacy_metadata.get("class"):
                ...
                log.debug(f"After legacy metadata: launcher={launcher}")
