...
from cfa_dagster import DynamicRunLauncher
        ...
        with patch(
            ...
            return_value=ExecutionConfig(
                launcher=SelectorConfig(
                    class_name="DefaultRunLauncher", config={}
                ),
...
def test_create_launcher_default_dev():
    ...
    try:
        ...
        try:
            ...
            with patch(
                "cfa_dagster.execution.run_launcher.DefaultRunLauncher"
...
def test_create_launcher_production_with_azure():
    ...
    try:
        # Mock the actual launcher creation to avoid complex setup
        with patch(
            "cfa_dagster.execution.run_launcher.AzureContainerAppJobRunLauncher"
        ) as mock_launcher_class:
            mock_launcher = Mock()
            mock_launcher_class.return_value = mock_launcher
...
def test_create_launcher_invalid_class_raises_error():
    ...
    object.__setattr__(
        execution_config,
        "launcher",
        SelectorConfig(class_name="InvalidLauncherClass", config={}),
    )
    ...
    try:
        with pytest.raises(
            RuntimeError, match="Invalid launcher class specified"
        ):
            launcher._create_launcher(execution_config)
    except Exception:
        # Skip this test if we can't properly mock the dependencies
...
def test_legacy_launcher_tags_parsing():
    ...
    tags = {
        "cfa_dagster/launcher": json.dumps(
            {"class": "DefaultRunLauncher", "config": {}}
        )
    }
...
def test_legacy_launcher_tags_invalid_json():
    ...
    tags = {"cfa_dagster/launcher": "{invalid json"}
...
def test_launch_run_integration(mock_context):
    ...
    try:
        ...
        with patch.object(
            launcher, "_get_launcher_config", return_value=execution_config
        ):
            with patch.object(
                launcher, "_create_launcher"
            ) as mock_create_launcher:
                mock_launcher = Mock()
