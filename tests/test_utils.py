...
def test_selector_config_bool():
    ...
    config_with_values = SelectorConfig(class_name="TestLauncher", config={"param": "value"})
    assert bool(config_with_values) is True
    
    # Config with None class_name should be falsy
    config_no_class = SelectorConfig(class_name=None, config={"param": "value"})
...
def test_execution_config_from_run_config_none():
    ...
    try:
        ...
        assert execution_config is None or (execution_config.launcher is None and execution_config.executor is None)
...
def test_execution_config_from_metadata():
    ...
    metadata = {
        "cfa_dagster/execution": Mock(spec=MetadataValue, value={
            "launcher": {"DefaultRunLauncher": {}},
            "executor": {"in_process_executor": {}}
        })
    }
...
def test_execution_config_to_run_tags():
    ...
    object.__setattr__(execution_config, 'executor', SelectorConfig(
        class_name="in_process_executor",
        config={}
    ))
...
def test_execution_config_to_run_tags_partial():
    ...
    expected_payload = {
        "launcher": {"DefaultRunLauncher": {"param": "value"}}
    }
    expected_json = json.dumps(expected_payload, separators=(',', ':'))
...
def test_execution_config_bool():
    ...
    object.__setattr__(config_with_launcher, 'launcher', SelectorConfig(class_name="DefaultRunLauncher", config={}))
    ...
    object.__setattr__(config_with_executor, 'executor', SelectorConfig(class_name="in_process_executor", config={}))
...
def test_execution_config_default_production():
    ...
    with patch('cfa_dagster.execution.utils.ExecutionConfig.default', return_value=ExecutionConfig(
        launcher=SelectorConfig(class_name="AzureContainerAppJobRunLauncher", config={}),
        ...
        assert default_config.launcher.class_name == "AzureContainerAppJobRunLauncher"
        assert default_config.executor.class_name == "multiprocess_executor"
